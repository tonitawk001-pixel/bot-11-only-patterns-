"""
HONEST OUT-OF-SAMPLE TEST: Regime detector vs baseline on NEVER-SEEN data.
Aug 2025 - Jan 2026 was NOT used to tune the regime threshold (35).
"""
import sys, os, warnings
warnings.filterwarnings('ignore')
OUT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.stdout = open(os.path.join(OUT_DIR, "honest_oos.txt"), 'w', encoding='utf-8')
...
print("=" * 80)
print("  HONEST OOS: Regime Detector on Never-Seen Period")
print(f"  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
print("=" * 80)

# Pull Aug 2025 - Jan 2026 (never used for regime tuning)
...
def run(use_regime):
    ...
    for i in range(400,n):
        ...
        if buy_ok:
            # Session filter: only in chop regime (if use_regime)
            if use_regime and not strong_trend and not (8<=hr<22):
                buy_ok=False
            if buy_ok:
        ...
        if not (not np.isnan(maf) and px>maf):
            # MACD gate + session: only in chop regime
            if use_regime and not strong_trend:
    ...
    return {'net':round(sum(trades),2),'trades':len(trades),
...
print(f"\n{'='*80}")
print("  RESULT: Aug 2025 - Jan 2026 (never seen by regime tuner)")
print(f"{'='*80}")
print(f"\n  BASELINE (no regime, no filters):")
print(f"    ${base['net']:+,.0f} | {base['trades']}t | WR={base['wr']}% | W{base['win_n']}/L{base['loss_n']}")
print(f"\n  REGIME DETECTOR (ADX>=35 aggressive, else filtered):")
print(f"    ${regime['net']:+,.0f} | {regime['trades']}t | WR={regime['wr']}% | W{regime['win_n']}/L{regime['loss_n']}")

diff = regime['net'] - base['net']
print(f"\n  Difference: ${diff:+,.0f}")
print(f"  Verdict: {'IMPROVEMENT' if diff>0 else 'NO IMPROVEMENT' if diff==0 else 'REGRESSION'}")
