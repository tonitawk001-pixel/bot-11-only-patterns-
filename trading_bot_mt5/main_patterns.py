    ...
    while True:
        ...
        try:
            ...
            if last_processed_m15_time is None or m15_time > last_processed_m15_time:
                ...
                # ── PATTERN ENGINE: direction from 3-year indicator-price patterns ──
                try:
                    ...
                    vol_ratio = (m15_vol / m15_vol_ma) if (m15_vol is not None and m15_vol_ma is not None and m15_vol_ma > 0) else None
                except Exception as e:
                ...
                # Evaluate all patterns (RSI momentum, Stoch bounce, BB band, volume, MACD, DI)
                pattern = evaluate_patterns(i15, m15_price, vol_ratio)
                raw_direction = pattern["direction"]
                original_dir = raw_direction
                direction = raw_direction
                if raw_direction == "NONE":
                    blocked_by = f"pattern_score_below_threshold_buy{pattern['buy_score']:.1f}_sell{pattern['sell_score']:.1f}"
                    logger.info(f"[PATTERN] no trade buy={pattern['buy_score']:.1f} sell={pattern['sell_score']:.1f} -> {pattern['reasons']}")
                else:
                    conf_score = pattern['buy_score'] if raw_direction == "BUY" else pattern['sell_score']
                    engine_result = {
                        "decision": True,
                        "confidence": int(min(95, 50 + conf_score * 10)),
                        "reason": ", ".join(pattern['reasons']),
                        "option": 0,
                        "checks": {"patterns": pattern['reasons']},
                    }
                    logger.info(f"[PATTERN] {raw_direction} buy={pattern['buy_score']:.1f} sell={pattern['sell_score']:.1f} -> {pattern['reasons']}")
