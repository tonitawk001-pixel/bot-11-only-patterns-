...
"""
ADAPTIVE STOCHASTIC: regime-conditioned rules from data.
Trend (ADX>=35): block SELL when K in 20-50 (selling into uptrend)
Chop (ADX<35): block BUY when K<35, block SELL when K 35-50
Verify across ALL data.
"""
import sys, os, warnings
warnings.filterwarnings('ignore')
OUT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.stdout = open(os.path.join(OUT_DIR, "adaptive_stoch_verify.txt"), 'w', encoding='utf-8')
...
print("=" * 80)
print("  ADAPTIVE STOCHASTIC VERIFY (regime-conditioned rules)")
print("=" * 80)
...
def run(use_adaptive_stoch):
    ...
    for i in range(400,n):
        ...
        # ADAPTIVE STOCH: chop regime blocks BUY when K<35
        if use_adaptive_stoch and buy_ok and not strong_trend and sk_i < 35:
            buy_ok = False
        ...
        if not (not np.isnan(maf) and px>maf):
            ...
            if px<bm[i]: checks+=1
            if checks>=3:
                # ADAPTIVE STOCH: block SELL in specific K ranges per regime
                block = False
                if use_adaptive_stoch:
                    if strong_trend and 20 <= sk_i <= 50:
                        block = True  # selling into uptrend
                    elif not strong_trend and 35 <= sk_i <= 50:
                        block = True  # chop SELL mid-K is bad
                if not block:
                    sld=atr*3.0; sl=px+sld; tp=px-sld*2.0
                    lots=max(0.01,round((bal*0.02)/(sld*100),2))
                    pos=('S',sl,tp,px,lots)
...
print(f"\n  BASELINE (no adaptive Stoch):")
print(f"    ${base['net']:+,.0f} | {base['trades']}t | WR={base['wr']}%")
print(f"\n  ADAPTIVE STOCH (regime-conditioned):")
print(f"    ${adpt['net']:+,.0f} | {adpt['trades']}t | WR={adpt['wr']}%")

diff = adpt['net'] - base['net']
print(f"\n  Difference: ${diff:+,.0f}")
print(f"  Verdict: {'IMPROVES' if diff>0 else 'HURTS'}")
