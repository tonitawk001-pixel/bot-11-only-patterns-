...
"""
DIRECTION-SPECIFIC ATR (VOLATILITY) TEST
Check if BUY and SELL need different volatility filters, like volume did.
Tests across all 2024-2026 data.
"""
import sys, os, warnings
warnings.filterwarnings('ignore')
OUT_DIR = os.path.dirname(os.path.abspath(__file__))
sys.stdout = open(os.path.join(OUT_DIR, "atr_direction.txt"), 'w', encoding='utf-8')
...
print("=" * 80)
print("  DIRECTION-SPECIFIC ATR (VOLATILITY) TEST")
print("=" * 80)
...
def run(buy_min, buy_max, sell_min, sell_max):
    """ATR% range for BUY and SELL separately."""
    ...
    for i in range(300, n):
        ...
        if trend_up:
            if not (buy_min <= ap <= buy_max): continue
            sl=px-atr*2.0; tp=px+atr*4.0
            lots=max(0.01,round((bal*0.02)/(atr*2.0*100),2))
            pos=['B',sl,tp,px,lots]
        else:
            if not (sell_min <= ap <= sell_max): continue
            sl=px+atr*2.0; tp=px-atr*4.0
            lots=max(0.01,round((bal*0.02)/(atr*2.0*100),2))
            pos=['S',sl,tp,px,lots]
...
print(f"\n  BASELINE (no ATR filter): ${base['net']:+,.0f} | {base['trades']}t | WR={base['wr']}%")

# BUY-only ATR filters (SELL unfiltered)
print(f"\n{'='*80}")
print("  BUY-ONLY ATR FILTERS (SELL unfiltered)")
print(f"{'='*80}")
buy_tests = [
    (0.1, 100, "buy ATR% >= 0.1 (skip dead)"),
    (0.1, 0.5, "buy ATR% 0.1-0.5 (skip extreme)"),
    (0.1, 0.3, "buy ATR% 0.1-0.3 (low-med)"),
    (0.2, 0.5, "buy ATR% 0.2-0.5 (med-high)"),
]
...
# SELL-only ATR filters (BUY unfiltered)
print(f"\n{'='*80}")
print("  SELL-ONLY ATR FILTERS (BUY unfiltered)")
print(f"{'='*80}")
sell_tests = [
    (0.1, 100, "sell ATR% >= 0.1 (skip dead)"),
    (0.1, 0.5, "sell ATR% 0.1-0.5"),
    (0.1, 0.3, "sell ATR% 0.1-0.3 (low-med)"),
    (0.2, 0.5, "sell ATR% 0.2-0.5 (med-high)"),
]
...
for bmin, bmax in [(0,100),(0.1,100),(0.1,0.5),(0.1,0.3)]:
    for smin, smax in [(0,100),(0.1,100),(0.1,0.5),(0.1,0.3)]:
